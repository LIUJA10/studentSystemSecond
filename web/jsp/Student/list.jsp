...
<SCRIPT language=javascript>
	...
	function selectSudent(student_id,student_name){
		//获得添加页面的window对象
		var win = window.opener;
		//获得添加页面的document对象
		var doc = win.document;
		//获得隐藏域,和 文本框,并赋值
		doc.getElementById("student_id").value=student_id;
		doc.getElementById("student_name").value=student_name;
		//关闭当前窗口
		window.close();
				...
				<TR>
					...
					<TD vAlign=top width="100%" bgColor=#ffffff>
						<TABLE cellSpacing=0 cellPadding=5 width="100%" border=0>
							<TR>
								<TD class=manageHead>当前位置：学生信息管理 &gt; 学生列表</TD>
							</TR>
							<TR>
								<TD height=2></TD>
							</TR>
							...
							<TBODY>
								<TR>
									...
									<FORM id="pageForm" name="studentForm"
										action="${pageContext.request.contextPath }/TXueshengAction_list"
										method=post>
										<!-- 隐藏域.当前页码 -->
											...
											<TBODY>
												<TR>
													...
													<TD><INPUT class=textbox id=sChannel2
														style="WIDTH: 80px" maxLength=50 name="student_name" value="${param.userName}"></TD>
										...
										<TABLE id=grid
											...
											<TBODY>
												...
												<TR 		
													...
													<TD>
														<s:property value="#student.xuehao" />
													</TD>
													<TD>
													<s:property value="#student.userName" />
													</TD>
													<TD>
													<s:property value="#student.xingbie" />
													</TD>
													<TD>
													<s:property value="#student.nianling" />
													</TD>
													<TD>
													<s:if test="#parameters.select==null">
														<a href="${pageContext.request.contextPath }/TXueshengAction_toEdit?xs_id=<s:property value="#tXuesheng.xs_id" />">修改</a>
														&nbsp;&nbsp;
														<a href="${pageContext.request.contextPath }/StudentServlet?method=delete&xs_id=${tXuesheng.xs_id}">删除</a>
													</s:if>
													<s:else>
														<input type="button" value="选择" onclick="selectStudent(<s:property value="#tXuesheng.xs_id" />,'<s:property value="#tXuesheng.userName" />')" />
													</s:else>
													</TD>
												</TR>
												</s:iterator>
